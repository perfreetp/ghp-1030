...
function getAuthor(opts) {
  if (opts && opts.author) return opts.author;
  if (process.env.KYG_AUTHOR) return process.env.KYG_AUTHOR;
  try {
    const os = require('os');
...
function appendHistory(action, entryIds, summary, author) {
