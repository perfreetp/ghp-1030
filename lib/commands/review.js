...
const { loadEntries, getStoreDir, getMissingFields, REQUIRED_FIELDS, OPTIONAL_FIELDS, loadChecklistState, saveChecklistState } = require('../store');
...
function review(opts) {
  ...
  if (opts.output) {
    const prevState = loadChecklistState();
    const newState = {};
    ...
    for (const [field, items] of byField) {
      ...
      for (const item of items) {
        ...
        newState[key] = { owner, note, entryName: item.name, entryVersion: item.version };
      }
      lines.push('---');
      lines.push('');
    }
