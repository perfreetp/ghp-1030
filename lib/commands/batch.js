...
const { loadEntries, saveEntries, appendHistory, getAuthor } = require('../store');
...
function batch(opts) {
  ...
  const actions = [];
  if (opts.setTag) actions.push(`+tag:${opts.setTag}`);
  if (opts.setCompany) actions.push(`company=${opts.setCompany}`);
  if (opts.setYear) actions.push(`year=${opts.setYear}`);
  appendHistory('batch', entries.map(e => e.id), `批量更新 ${changed} 条: ${actions.join(', ')}`, getAuthor(opts));
