...
const { STORE_DIR, getStoreDir, isInitialized, loadEntries, saveEntries, appendHistory, cleanupDanglingRelations } = require('../store');
...
function unpack(opts) {
  ...
  if (mode === 'overwrite') {
    ...
    doMerge(importEntries, importMeta, sourcePath, stat);
  } else {
...
function readFromDir(sourceDir) {
  ...
  return readFromDataDir(dataDir);
}
...
function doOverwrite(sourcePath, stat) {
  ...
  appendHistory('unpack-overwrite', [], `从 ${path.basename(sourcePath)} 覆盖导入`);

  console.log(chalk.green('✓ 馆藏已覆盖导入'));
  console.log(chalk.cyan('现在可以使用 kyg report、kyg scan、kyg compare 等命令继续操作'));
}

function doMerge(importEntries, importMeta, sourcePath, stat) {
  const targetDir = getStoreDir();
  ...
  for (const impEntry of importEntries) {
    const matchKey = `${(impEntry.name || '').toLowerCase()}|${(impEntry.version || '').toLowerCase()}|${(impEntry.company || '').toLowerCase()}`;
    const localMatch = localEntries.find(e =>
      `${(e.name || '').toLowerCase()}|${(e.version || '').toLowerCase()}|${(e.company || '').toLowerCase()}` === matchKey
    );
  ...
  if (conflicts === 0) {
    ...
    appendHistory('unpack-merge', localEntries.slice(-imported).map(e => e.id), `从 ${path.basename(sourcePath)} 合并导入 ${imported} 条`);
  ...
  for (let i = 0; i < conflictList.length; i++) {
    ...
    const company = loc.company ? ` [${loc.company}]` : '';
    console.log(chalk.white(`冲突 ${i + 1}: ${loc.name || '(未命名)'}${version}${company}`));
    console.log(chalk.gray(`  本地 ID: ${loc.id} | 导入 ID: ${imp.id}`));
  ...
  const strategy = opts_global_merge_strategy || 'merge';
  resolveConflicts(localEntries, conflictList, strategy);

  saveEntries(localEntries);
  appendHistory('unpack-merge', [], `从 ${path.basename(sourcePath)} 合并导入 ${imported} 条, 解决 ${conflicts} 组冲突(${strategy})`);

  console.log(chalk.green(`✓ 合并完成: 新增 ${imported} 条，解决 ${conflicts} 组冲突`));
}

let opts_global_merge_strategy = 'merge';
...
function unpackMain(opts) {
  if (opts.strategy) {
    opts_global_merge_strategy = opts.strategy;
  }
  unpack(opts);
}

module.exports = unpackMain;
