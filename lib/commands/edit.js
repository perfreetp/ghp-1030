...
const { loadEntries, saveEntries, getMissingFields, appendHistory, getAuthor } = require('../store');
...
function edit(opts) {
  ...
  const changes = [];
  if (opts.name !== undefined) changes.push(`name=${opts.name}`);
  if (opts.version !== undefined) changes.push(`version=${opts.version}`);
  if (opts.company !== undefined) changes.push(`company=${opts.company}`);
  if (opts.year !== undefined) changes.push(`year=${opts.year}`);
  if (opts.description !== undefined) changes.push('description');
  if (opts.link !== undefined) changes.push('link');
  if (opts.interview !== undefined) changes.push('interview');
  if (opts.dispute !== undefined) changes.push('dispute');
  if (opts.features !== undefined) changes.push('features');
  if (opts.tags !== undefined) changes.push('tags');
  appendHistory('edit', [entry.id], `修改 ${entry.name || '(未命名)'}: ${changes.join(', ')}`, getAuthor(opts));
