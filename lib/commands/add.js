...
function add(opts) {
  ...
  if (opts.screenshot) {
    ...
    if (fs.existsSync(src)) {
      const dest = resolveScreenshotDest(getStoreDir(), src);
      fs.copyFileSync(src, dest);
      entry.screenshot = path.join('screenshots', path.basename(dest));
      console.log(chalk.green('✓ 截图已复制:'), path.basename(dest));
    } else {
