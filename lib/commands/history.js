...
function history(opts) {
  ...
  if (opts.author) {
    records = records.filter(r => r.author === opts.author);
  }
  ...
  if (opts.author) console.log(chalk.gray(`操作者: ${opts.author}`));
  console.log();

  for (const r of records) {
    const time = new Date(r.timestamp).toLocaleString('zh-CN');
    const author = r.author || 'unknown';
    console.log(chalk.white(`[${time}] ${r.action}`), chalk.blue(`@${author}`));
