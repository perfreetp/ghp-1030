...
const { loadEntries, saveEntries, appendHistory, cleanupDanglingRelations } = require('../store');
...
function addRelation(opts) {
  ...
  appendHistory('relate-add', [opts.id, opts.target], `${source.name} →[${TYPE_LABELS[type]}]→ ${target.name}`);
...
function deleteRelation(opts) {
  ...
  if (target && target.relations) {
    ...
    target.relations = target.relations.filter(r => {
      ...
      if (reverseTypes.length > 0 && !reverseTypes.includes(r.type)) return true;
      if (reverseTypes.length === 0) return true;
      return false;
    });
  }
  ...
  appendHistory('relate-delete', [opts.id, opts.target], `删除 ${source.name} → ${target ? target.name : opts.target} 的关系`);
